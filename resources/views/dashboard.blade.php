        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="bg-surface border border-dashed border-denim/30 rounded-md p-5">
                    ...
                    <p class="font-mono text-3xl font-medium text-denim mt-1">{{ $totalDistributions }}</p>
                </div>
                <div class="bg-surface border border-dashed border-denim/30 rounded-md p-5">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-surface border border-dashed border-denim/30 rounded-md p-6">
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="font-mono">
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="py-4 text-center text-ink/40 font-sans">Belum ada data distribusi.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="bg-surface border border-dashed border-denim/30 rounded-md p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('predictions.index') }}" class="font-sans text-sm text-thread hover:text-thread-light">Lihat semua</a>
                    </div>
                    <ul class="divide-y divide-denim/10">
                        @forelse ($latestPredictions as $product)
                            <li class="py-2 flex items-center justify-between">
                                <span class="font-sans text-ink">{{ $product->name }}</span>
                                @if ($product->predicted_label === 'Laris')
                                    <span class="relative inline-flex items-center gap-1.5 pl-3 pr-2.5 py-1 bg-moss/10 text-moss text-xs font-mono rounded-r-md before:content-[''] before:w-1.5 before:h-1.5 before:rounded-full before:bg-moss">
                                        {{ $product->predicted_label }}
                                    </span>
                                @else
                                    <span class="relative inline-flex items-center gap-1.5 pl-3 pr-2.5 py-1 bg-brick/10 text-brick text-xs font-mono rounded-r-md before:content-[''] before:w-1.5 before:h-1.5 before:rounded-full before:bg-brick">
                                        {{ $product->predicted_label }}
                                    </span>
                                @endif
                            </li>
                        @empty
                            <li class="py-4 text-center text-ink/40 font-sans text-sm">
